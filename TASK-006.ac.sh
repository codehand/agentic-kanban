...
test_files=$(grep -rEl 'evidence|selfcheck' server/test --include='*.test.ts' --include='*.spec.ts' 2>/dev/null || true)
[ -n "$test_files" ] || fail "no evidence/selfcheck test file under server/test"
