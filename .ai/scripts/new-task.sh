...
# When TASK_HUB_URL is set, create the task on the server via MCP tool task.create
# in addition to creating the local skeleton files.
...
# Create the task on the Task Hub server via MCP tool task.create.
server_task_create() { # TITLE KEY REPOS
  [ -n "${TASK_HUB_URL:-}" ] && type mcp_call &>/dev/null || return 0
  local title="$1" key="$2" repos="$3"
  # Convert space-separated repos to JSON array for the MCP tool.
  local repos_json
  repos_json="$(printf '%s\n' $repos | jq -Rn '[inputs | select(length>0)]')"
  mcp_call "task.create" "$(jq -nc --arg t "$title" --arg k "$key" --argjson r "$repos_json" \
    '{project:"default", key:$k, title:$t, repos:$r}')" \
    || warn "server: task.create failed (offline fallback OK)"
}
...
# --- Thin-client: push task creation to server via MCP task.create (P8 / §12) ---
server_task_create "$TITLE" "$ID" "$REPOS"
